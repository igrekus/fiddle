...
from fiddle import is_impossible, is_straight, is_kind_4, is_kind_3, is_one_pair, is_two_pairs, is_full_house


def test_is_impossible():
    expect(is_impossible([1, 1, 1, 1, 1])).to_equal(True)
    expect(is_impossible([2, 1, 1, 1, 1])).to_equal(False)
...
def test_is_one_pair():
    expect(is_one_pair([1, 2, 3, 4, 4])).to_equal(True)
    expect(is_one_pair([1, 2, 3, 3, 4])).to_equal(True)
    expect(is_one_pair([1, 1, 3, 4, 4])).to_equal(False)
    expect(is_one_pair([1, 5, 3, 4, 2])).to_equal(False)
